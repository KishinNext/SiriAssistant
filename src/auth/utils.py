...
def get_project_path() -> Path:
    ...
    :return: A Path object of the project path.
    """
...
def get_config_file_path(env: str) -> Path:
    ...
    return current_path.parent / 'auth' / 'config' / config_file_name


def get_current_environment(default: str = 'development') -> str:
    """
    Gets the current environment from the 'ENVIRONMENT' environment variable, or returns a default value.
...
def get_secrets() -> Dict[str, Any]:
    ...
    secrets = load_yaml_file(get_config_file_path(environment)).get('secrets')
    if secrets is None:
        logging.warning(f"'secrets' not found in the configuration for {environment}.")
    return secrets
